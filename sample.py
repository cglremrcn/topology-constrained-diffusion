...
def p_sample(model,x,t,t_index):
    ...
    model_mean = sqrt_recip_alpha_t * (x - beta_t * model(x,t_tensor) / sqrt_one_minus_alpha_cumprod_t)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Error: 'diffusion_model.pth' not found. Run train.py first!")
        exit()
    ...
    with torch.no_grad():
        generated_data = sample(model, shape=(1, 1000, 2))

    
    data = generated_data[0].cpu().numpy() 
    
    plt.figure(figsize=(6,6))
    plt.scatter(data[:,0], data[:,1], s=5, c='purple', alpha=0.6)
